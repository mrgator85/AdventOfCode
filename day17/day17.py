...
def grow(m):
    #grow 
    maxx = max([x[0] for x in m.keys() if x[2] == 0 and x[1] == 0])
    minx = min([x[0] for x in m.keys() if x[2] == 0 and x[1] == 0])
    maxy = max([x[1] for x in m.keys() if x[2] == 0 and x[0] == 0])
    miny = min([x[1] for x in m.keys() if x[2] == 0 and x[0] == 0])
    maxz = max([x[2] for x in m.keys() if x[1] == 0 and x[0] == 0])
    minz = min([x[2] for x in m.keys() if x[1] == 0 and x[0] == 0])
    for z in range(minz-1, maxz+2, 1):
        for y in range(miny-1, maxy+2, 1):
            for x in range(minx-1, maxx+2, 1):
                if((x, y, z) not in m.keys()):
                    m[(x,y,z)] = False
def buildMap(s):
    r = s.strip().split('\n')
    midy = int(len(r)/2)
